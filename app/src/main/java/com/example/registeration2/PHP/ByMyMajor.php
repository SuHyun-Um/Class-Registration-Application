<?php
    ...
    $result = mysqli_query($con, "SELECT COURSE.courseID, COURSE.courseGrade, COURSE.courseTitle, COURSE.courseProfessor, COURSE.courseCredit,
      COURSE.courseDivide, COURSE.coursePersonnel, COURSE.courseTime FROM SCHEDULE, COURSE, SU WHERE SU.userMajor IN (SELECT userMajor FROM SU
      WHERE userID ='$userID') AND SU.userID = SCHEDULE.userID AND SCHEDULE.courseID = COURSE.courseID GROUP BY
